...
Смотрите лог для деталей."""
    ...
    def match_text_to_rows(self, extracted_text, table_rows, file_path, headers):
        ...
        # Показываем AI меньше строк для ускорения
        # Ограничиваем до 30 строк
        rows_description = []
        for row_num, row_data in list(table_rows.items())[:30]:
            # Объединяем все данные строки в одну строку
            row_text = " | ".join([f"{col}: {val}" for col, val in row_data.items()])
            # Сокращаем до 100 символов
            rows_description.append(f"Строка {row_num}: {row_text[:100]}")

        rows_text = "\n".join(rows_description)

        # Логируем размер промпта для отладки
        prompt_size = len(rows_text) + len(extracted_text[:1000])
        self.log(f"   Размер данных для AI: ~{prompt_size} символов (показано {len(rows_description)} строк из {len(table_rows)})")

        prompt = f"""ВЕРНИ ТОЛЬКО JSON БЕЗ ОБЪЯСНЕНИЙ!

Задача: исправь ошибки OCR и найди подходящие строки.

Таблица (первые 30 строк):
{rows_text[:3500]}

Текст OCR:
{extracted_text[:1000]}

Исправь "СМУ Nglю"→"СМУ №1", "мерог"→"мероприятий" и подобные.

ВЕРНИ ТОЛЬКО JSON:
{{
  "matched_rows": [13, 82],
  "target_column": "Свидетельства",
  "extracted_data": "Описание с исправленным текстом",
