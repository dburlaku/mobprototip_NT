...
Смотрите лог для деталей."""
    ...
    def create_table_index(self, table_rows):
        ...
        Returns:
            str: Детальное описание содержимого строк таблицы
        """
        self.log("🗂️ Создание индекса тем таблицы (один раз)...")

        # Формируем ПОЛНОЕ описание всех строк (не обрезаем!)
        rows_description = []
        for row_num, row_data in table_rows.items():
            # Берем ПОЛНЫЙ текст из колонки B (или всех колонок)
            row_text = " | ".join([f"{val}" for val in row_data.values()])
            # Увеличиваем лимит с 80 до 400 символов, чтобы показать полный контекст
            rows_description.append(f"{row_num}: {row_text[:400]}")

        rows_text = "\n".join(rows_description)

        # НОВЫЙ ПРОМПТ: просим AI создать структурированный индекс с полным контекстом
        prompt = f"""Анализируй таблицу аудита и создай детальный индекс.

ТАБЛИЦА ({len(table_rows)} строк):
{rows_text[:25000]}

ЗАДАЧА: Создай структурированный индекс в формате:

СТРОКА [номер]: [краткое описание темы/вопроса]
- Что должно быть в ответе: [описание ожидаемого содержания]

Пример:
СТРОКА 57: Заключение по управлению документами
- Что должно быть: заключения, названия нормативных документов, процедуры СМ

СТРОКА 82: Компетентность персонала
- Что должно быть: удостоверения, сертификаты, приказы

Создай такой индекс для ВСЕХ строк. ТОЛЬКО индекс, БЕЗ вступлений!"""

        try:
            response = self.query_ollama(prompt)
            if response and len(response) > 100:
                self.log(f"✅ Индекс создан ({len(response)} символов, ~{len(response.splitlines())} строк)")
                return response
            else:
    ...
    def match_text_to_rows(self, extracted_text, table_rows, file_path, headers, table_index=None):
        ...
        if table_index:
            prompt = f"""Ты - эксперт по заполнению таблиц аудита. ВЕРНИ ТОЛЬКО JSON, ничего больше!

ИНДЕКС ТАБЛИЦЫ АУДИТА:
{table_index[:10000]}

ТЕКСТ ИЗ ДОКУМЕНТА (может содержать ошибки OCR):
{extracted_text[:1500]}
...
2. Найди строки таблицы, куда подходит этот текст по СМЫСЛУ
   ВАЖНО: Ищи СТРОКИ ДЛЯ ЗАПОЛНЕНИЯ, НЕ заголовки!
   Если текст про "политику безопасности" - найди строку, где СПРАШИВАЕТСЯ про политику

3. Верни СТРОГО этот JSON (без комментариев, без объяснений до/после):
{{
  "matched_rows": [57, 82],
  "target_column": "Свидетельства",
  "extracted_data": "Исправленный текст из документа",
  "explanation": "Текст содержит информацию о..."
}}
...
ВЕРНИ ТОЛЬКО JSON!"""
            ...
            for row_num, row_data in table_rows.items():
                ...
                rows_description.append(f"{row_num}: {row_text[:300]}")

            rows_text = "\n".join(rows_description)

            prompt = f"""Ты - эксперт по заполнению таблиц аудита. ВЕРНИ ТОЛЬКО JSON!

ТАБЛИЦА АУДИТА ({len(table_rows)} строк):
{rows_text[:18000]}
...
ЗАДАЧА:
1. Исправь ошибки распознавания (СМУ Nglю→СМУ №1, мерог→мероприятий)
2. Найди строки таблицы, куда подходит этот текст по СМЫСЛУ
   ВАЖНО: Ищи СТРОКИ ДЛЯ ЗАПОЛНЕНИЯ (где должен быть ответ), НЕ заголовки!
3. Верни СТРОГО этот JSON (без текста до/после):
{{
  "matched_rows": [57, 82],
  "target_column": "Свидетельства",
  "extracted_data": "Исправленный текст",
