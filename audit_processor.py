...
Смотрите лог для деталей."""
    ...
    def create_table_index(self, table_rows):
        ...
        Просто список всех строк с полным текстом (без AI-обработки)

        Returns:
            str: Список строк с их содержимым
        """
...
2. Найди 1-3 строки, где НУЖНО ЗАПОЛНИТЬ ответ этим текстом (не заголовки!)
3. Верни JSON БЕЗ переносов строк в тексте:

{{"matched_rows":[57,82],"target_column":"Свидетельства","extracted_data":"текст без переносов строк","explanation":"краткое объяснение"}}
...
ИНСТРУКЦИИ:
1. Исправь ошибки OCR: "СМУ Nglю"→"СМУ №1", удали лишние пробелы
2. Найди 1-3 строки, где НУЖНО ЗАПОЛНИТЬ ответ (не заголовки!)
3. Верни JSON БЕЗ переносов строк:

{{"matched_rows":[57,82],"target_column":"Свидетельства","extracted_data":"текст без переносов","explanation":"объяснение"}}
