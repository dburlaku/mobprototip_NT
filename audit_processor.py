...
Смотрите лог для деталей."""
    ...
    def create_table_index(self, table_rows):
        ...
        for row_num, row_data in table_rows.items():
            ...
            if not is_header:
                # Показываем до 45 символов на строку (оптимальный баланс: влезут все 152 строки)
                rows_description.append(f"Строка {row_num}: {row_text[:45]}")
                fillable_rows_count += 1

        index_text = "\n".join(rows_description)

        # Устанавливаем лимит 8500 символов (чтобы вместить ВСЕ 152 строки)
        # При 45 символах на строку + 12 префикс = 57*152 = ~8700, обрежется до 8500
        max_index_size = 8500
