...
Смотрите лог для деталей."""
    ...
    def match_text_to_rows(self, extracted_text, table_rows, file_path, headers):
        ...
        # Показываем AI ВСЕ строки (не фильтруем по "?")
        # Ограничиваем до 50 строк для оптимизации размера промпта
        rows_description = []
        for row_num, row_data in list(table_rows.items())[:50]:
            # Объединяем все данные строки в одну строку
            row_text = " | ".join([f"{col}: {val}" for col, val in row_data.items()])
            # Сокращаем до 120 символов для уменьшения размера
            rows_description.append(f"Строка {row_num}: {row_text[:120]}")

        rows_text = "\n".join(rows_description)

        # Логируем размер промпта для отладки
        prompt_size = len(rows_text) + len(extracted_text[:1200])
