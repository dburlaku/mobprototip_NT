...
Смотрите лог для деталей."""
    ...
    def match_text_to_rows(self, extracted_text, table_rows, file_path, headers):
        ...
        # Фильтруем только строки с ВОПРОСАМИ (содержащие "?")
        question_rows = {}
        for row_num, row_data in table_rows.items():
            # Проверяем, есть ли "?" в любой колонке строки
            row_text = " ".join([str(val) for val in row_data.values()])
            if "?" in row_text:
                question_rows[row_num] = row_data

        self.log(f"   Отфильтровано строк с вопросами: {len(question_rows)} из {len(table_rows)}")

        # Формируем описание строк с вопросами для AI (показываем до 100 строк)
        rows_description = []
        for row_num, row_data in list(question_rows.items())[:100]:
...
- Колонка B: Вопросы/Информация
- Колонка C: Свидетельства (сюда нужно вставлять извлеченные данные)

СУЩЕСТВУЮЩИЕ СТРОКИ С ВОПРОСАМИ (до 100 строк):
{rows_text[:5000]}
...
2. Найди строки, вопросы в которых относятся к извлеченному тексту по смыслу
3. Извлеченные данные нужно вставить в колонку "Свидетельства"
...
- target_column ВСЕГДА должна быть "Свидетельства"
